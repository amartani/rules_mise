"""End-to-end test for a conda-backend postgres: initdb, start, query, stop."""
...
REPO = "mise/tools/conda_postgresql"
tool = r.Rlocation(os.path.join(REPO, "tool"))

# The conda tool wrapper dispatches to the conda prefix's bin/ via its first
# argument (e.g. `tool psql --version`), while also supporting argv[0]
# dispatch for per-binary symlinks. Wrap it so the rest of the test reads
# like the old pkgx test which had one target per binary.
psql = [tool, "psql"]
initdb = [tool, "initdb"]
pg_ctl = [tool, "pg_ctl"]
pg_isready = [tool, "pg_isready"]
