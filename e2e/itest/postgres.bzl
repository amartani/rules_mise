"""Launches the pkgx postgres closure as an itest service executable.
...
drops root privileges like e2e/pkgx does (postgres refuses to run as root),
then execs postgres. Extra service `args` (e.g. the autoassigned `-p` port)
are forwarded via "$@".
"""

def _postgres_server_impl(ctx):
    postgres = ctx.executable.postgres
    initdb = ctx.executable.initdb
...
exec "$POSTGRES" -D "$PGDATA" -c "listen_addresses=127.0.0.1" -c "unix_socket_directories=$SOCKDIR" "$@"
""".format(
        postgres_short = postgres.short_path,
        initdb_short = initdb.short_path,
    )
    ctx.actions.write(output = out, content = content, is_executable = True)
    runfiles = ctx.runfiles(files = [out])
    runfiles = runfiles.merge(ctx.attr.postgres[DefaultInfo].default_runfiles)
    runfiles = runfiles.merge(ctx.attr.initdb[DefaultInfo].default_runfiles)
    return [DefaultInfo(executable = out, runfiles = runfiles)]

postgres_server = rule(
    implementation = _postgres_server_impl,
    attrs = {
        "postgres": attr.label(executable = True, cfg = "target", mandatory = True),
        "initdb": attr.label(executable = True, cfg = "target", mandatory = True),
    },
    executable = True,
)
