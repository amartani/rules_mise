...
Downloads honor private registries via netrc (`NETRC` env / user netrc matched against per-URL auth patterns), and forward `headers` on Bazel versions that support `download_has_headers_param`.

### 4. Hub Repository

The `_mise_hub` repository itself contains no binaries — it declares toolchains and the user-facing rules:

- `toolchain_info.bzl` — `toolchain_info` rule (`executable`, `os`, `cpu`, `ext`) plus `declare_toolchain()`, which wires each `tool_repo` executable into `exec` + `target` toolchains constrained on `@platforms//os:{os}` / `@platforms//cpu:{cpu}`.
...
- `tools/<tool>/tool.bzl` — a `tool` rule that resolves `//tools/<tool>:toolchain_type` and symlinks the selected toolchain executable to its own output.
...
├── hub.bzl                  # bzlmod_hub / workspace_hub, tool_repo, _mise_hub
├── cwd.bzl / workspace_root.bzl  # public re-exports of the private wrappers
└── private/
    ├── lockfile.bzl         # mise.lock parsing, platform map, name sanitizing
...
4. **Repository fetching** (`tool_repo` impl): downloads/verifies/extracts per §3.
...
2. **Tool Sources**: `rules_mise` consumes `mise.toml`-managed tools with direct download URLs from supported backends.
3. **Architecture**: both use separate per-platform tool repos fronted by a toolchain hub; `rules_mise` additionally generates `:cwd` / `:workspace_root` wrappers.
